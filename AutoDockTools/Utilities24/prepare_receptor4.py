...
def prepare_receptor(
    ...
    delete_single_nonstd_residues: str = None,
    mode: str = "automatic",
    dictionary: str = None,
    unique_atom_names: bool = False,
):
    """Prepare a receptor for an AutoDock experiment.

    :param receptor_filename: supported file types include pdb,mol2,pdbq,pdbqs,pdbqt, possibly pqr,cif
    :param verbose: verbose output, defaults to False
    :param outputfilename: defaults to "molecule_name.pdbqt"
    :param repairs: type(s) of repairs to make:
        ...
        defaults to "None"
    :param charges_to_add: preserve all input charges ie do not add new charges,
        defaults to "gasteiger", addition of gasteiger charges
    :param preserve_charge_types: preserve input charges on specific atom types, eg -p Zn -p Fe, defaults to None
    :param cleanup: cleanup type:
        ...
        defaults to "nphs_lps_waters_nonstdres"
    :param delete_single_nonstd_residues: delete every nonstd residue from any chain,
        ...
        defaults to False
    :param mode: interactive, defaults to "automatic": outputfile is written with no further user input)
    :param dictionary:  file to contain receptor summary information, defaults to None
    :param unique_atom_names: assign each receptor atom a unique name: newname is original name plus its index(1-based), defaults to False
    """
    ...
    if unique_atom_names:  # added to simplify setting up covalent dockings 8/2014
        ...
        if verbose:
            print("renamed %d atoms: each newname is the original name of the atom plus its (1-based) uniqIndex" %
                  (len(mol.allAtoms)))
    preserved = {}
